...
class WishSystem:
    """ Manipulate with wished products """

    def get_or_create_wish(self, request):
        """ Get or create wish for customer """

        customer = AccountSystem.get_customer_account(request)
        wish = self._get_customer_wish(customer=customer)
        if not wish:
            wish = self._create_customer_wish(customer=customer)
        return wish

    def get_or_create_wish_for_anonymos(self):
        """  Get or create wish for anonymos customer """

        wish = self._get_wish_for_anonymos()
        if not wish:
            wish = self._create_wish_for_anonymos()
        return wish
    ...
    def add_product_to_wish(wish_product):
        """ Add product to wish """

        wish = wish_product.wish
    ...
    def remove_wish_product(wish_product):
